...
class IOdczytajListeObecnosci {
public:
    virtual void odczytajZPliku(string nazwaPliku) = 0;
};

class IZapiszListeObecnosci {
public:
...
class ListaObecnosci : public IOdczytajListeObecnosci {
private:
...
public:
    ...
    void odczytajZPliku(string nazwaPliku) override;
};
...
void ListaObecnosci::drukujListe() {
    if (licznik != 0) {
        if (!nazwaPrzedmiotu.empty()) {
            cout << "=== PRZEDMIOT: " << nazwaPrzedmiotu << " (Liczba zajec: " << liczbaZajec << ") ===\n";
        }
...
void ListaObecnosci::odczytajZPliku(string nazwaPliku) {
    ...
    getline(plik, nazwaPrzedmiotu);

    int rozmiarZPliku;
    plik >> liczbaZajec >> rozmiarZPliku;

    for (int i = 0; i < rozmiarZPliku; ++i) {
        int tempIndeks, tempWartosc;
        string tempImie, tempNazwisko;

        plik >> tempIndeks >> tempImie >> tempNazwisko >> tempWartosc;

        Student* nowyStudent = new Student();
        nowyStudent->setIndex(tempIndeks);
        nowyStudent->setImie(tempImie);
        nowyStudent->setNazwisko(tempNazwisko);


        dodajOsobe(nowyStudent);

        if (licznik > 0) {
            obecnosc[licznik - 1] = (tempWartosc > 0);
        }
    }

    plik.close();
    cout << "Wczytano " << rozmiarZPliku << " osob do listy przedmiotu: " << nazwaPrzedmiotu << ".\n";
}
...
void Interfejs::menu() {
    ...
    cout << "7. Wczytaj z pliku (ListaObecnosci.txt)." << endl;
...
int Interfejs::uruchom() {
    ...
    while (1) {
        ...
        case 6:
            ...
            break;
        case 7:
            listaAktywna->odczytajZPliku("ListaObecnosci.txt");
            break;
        case 0:
            return 0;
            break;
